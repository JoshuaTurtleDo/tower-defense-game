...
canvas.addEventListener("pointerdown", event => {
  // Desktop keeps right-drag orbiting; touch uses a natural one-finger drag.
  // A short touch is still allowed through as a tower-placement tap.
...
function finishCameraDrag(event, cancelled = false) {
  ...
  cameraDrag.suppressClick = !cancelled && cameraDrag.dragged;
...
document.getElementById("zoomOutButton").addEventListener("click", () => graphics3D.zoomBy(180));
document.getElementById("zoomInButton").addEventListener("click", () => graphics3D.zoomBy(-180));
